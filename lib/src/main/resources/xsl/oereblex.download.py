...
class OereblexDownloader:
    def run(self, xtf_path, output_directory, settings):
        print("OereblexDownloader.run")
        ...
        for geolink_id in oereblex_geolink_unique.keys():
            for language in oereblex_geolink_unique[geolink_id].keys():
                ...
                request = urllib.request.urlopen(geolink) # requests module funktioniert nicht mit GraalVM
                if request.getcode() < 200 and request.getcode > 299:
                    raise IOError("Could download documents for {}".format(geolink))
                logging.info("Successfully downloaded from {}".format(geolink))
                #xml =  request.read().decode(request.headers.get_content_charset())
                xml =  request.read() # https://lxml.de/FAQ.html#why-can-t-lxml-parse-my-xml-from-unicode-strings
...
polyglot.export_value("OereblexDownloader", OereblexDownloader)
